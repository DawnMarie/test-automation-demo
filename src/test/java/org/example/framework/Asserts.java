...
public class Asserts {
    ...
    public void assertLocationSearchResponse(JsonNode response, String title, String locationType, Boolean isLatLongSearch) {
        ...
        for(int i = 0; i < response.size(); i++) {
            TS.asserts().equalsTo("Assert title", title, response.get(i).get("title").asText());
            TS.asserts().equalsTo("Assert location type", locationType, response.get(i).get("location_type").asText());
            TS.asserts().isNotEmpty("Assert woeid", response.get(i).get("woeid").asInt());
            ...
            if (isLatLongSearch) {
                ...
                TS.asserts().isNotEmpty("Assert distance", response.get(i).get("distance").asInt());
            }
        }
    }
    ...
    public void assertLocationResponse(JsonNode response, String title, String locationType) {
        ...
        TS.asserts().isNotEmpty("Assert woeid", response.get("woeid").asInt());
    ...
    public void assertConsolidatedWeatherResponse(JsonNode response) {
        for (int i = 0; i < response.size(); i++) {
            TS.asserts().isNotEmpty("Assert id", response.get(i).get("id").asInt());
            ...
            TS.asserts().isNotEmpty("Assert min_temp", response.get(i).get("min_temp").asInt());
            TS.asserts().isNotEmpty("Assert max_temp", response.get(i).get("max_temp").asInt());
            TS.asserts().isNotEmpty("Assert the_temp", response.get(i).get("the_temp").asInt());
            TS.asserts().isNotEmpty("Assert wind_speed", response.get(i).get("wind_speed").asDouble());
            TS.asserts().isNotEmpty("Assert wind_direction", response.get(i).get("wind_direction").asDouble());
            TS.asserts().isNotEmpty("Assert air_pressure", response.get(i).get("air_pressure").asDouble());
            TS.asserts().isNotEmpty("Assert humidity", response.get(i).get("humidity").asInt());
            TS.asserts().isNotEmpty("Assert visibility", response.get(i).get("visibility").asDouble());
            TS.asserts().isNotEmpty("Assert predictability", response.get(i).get("predictability").asInt());
        }
    }
}
